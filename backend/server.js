...
app.get('/generateWorkout', (req, res) => {
  ...
  const targetMuscles = target.split(','); // Split targets by comma for multiple muscles
  const availableTime = parseInt(time, 10);

  // Filter exercises by each target muscle
  ...
  // Divide time evenly across target muscles
  ...
  targetMuscles.forEach((muscle) => {
    ...
    const availableExercises = [...muscleExercises[muscle]]; // Clone exercise list

    // Select exercises until time for this muscle is filled
    while (muscleTime < timePerMuscle && availableExercises.length > 0) {
      const exercise = availableExercises.shift(); // Take the first exercise
...
// Start the Server
const PORT = 5001; // Change to an available port
app.listen(PORT, () => {
